...
class Account extends React.Component {
    ...
    if (value !== null && value !== "") {
      ...
      if (nameValid) {
        ...
        if (field === "fname") {
          ...
          await this.setState({
            error: "Error: Only alphabetical characters allowed in names!",
          });
          return false;
    ...
    if (password !== "") {
      ...
      if (pwd.length >= 8) {
        ...
        if (hasUpperCase && hasLowerCase && hasNumbers) {
          if (confirm) await this.setState({ passCheck: true });

          await this.setState({ editingPassword: true });

          return true;
        } else {
          if (confirm) await this.setState({ passCheck: false });
  ...
  async save() {
    ...
    if (this.state.password !== null) {
      ...
      if (!confirm) await this.setState({ passCheck2: false });
    ...
    if (this.state.nameChangedLast !== null) {
      if (
        this.state.nameChangedLast &&
        !this.changeName(this.state.fname, "lname")
      )
        return false;
    }
    ...
    if (this.state.nameChangedFirst !== null) {
      if (
        this.state.nameChangedFirst &&
        !this.changeName(this.state.lname, "fname")
      )
        return false;
    }
          ...
          <div class="container-fluid">
            ...
            {this.state.passCheck === false ? (
              ...
              >
                ...
                characters or length requirements!
              </div>
            ...
            <form id="user-update-form">
              ...
              <div class="form-row form-group">
                <div class="col">
                  ...
                  <input
                    ...
                    className={
                      this.state.fnameCheck === null
                        ? "form-control"
                        : this.state.fnameCheck === true
                        ? "form-control border border-success"
                        : this.state.fnameCheck === false
                        ? "form-control border border-danger"
                        : "form-control"
                    }
                    onChange={(event) =>
                      this.changeName(event.target.value, "updated-fname")
                    }
                    onBlur={(event) =>
                      this.changeName(event.target.value, "fname")
                    }
                ...
                <div class="col">
                  ...
                  <input
                    ...
                    id="lastname"
                    className={
                      this.state.lnameCheck === null
                        ? "form-control"
                        : this.state.lnameCheck === true
                        ? "form-control border border-success"
                        : this.state.lnameCheck === false
                        ? "form-control border border-danger"
                        : "form-control"
                    }
                    placeholder={this.state.lname}
                    onChange={(event) =>
                      this.changeName(event.target.value, "updated-lname")
                    }
                    onBlur={(event) =>
                      this.changeName(event.target.value, "lname")
                    }
                    readOnly={!this.state.edit}
                    required
                  />
              ...
              <div class="form-group">
                ...
                <input
                  ...
                  onBlur={(event) =>
                    this.validateEmail(event.target.value, true)
                  }
                  readOnly={!this.state.edit}
                />
              </div>
              ...
              <div class="form-group">
                ...
                <input
                  ...
                  placeholder="************"
                  onChange={(event) =>
                    this.validatePassword(event.target.value, false)
                  }
                  onBlur={(event) =>
                    this.validatePassword(event.target.value, true)
                  }
                  readOnly={!this.state.edit}
                />
              </div>
              {this.state.editingPassword ? (
                <div class="form-group">
                  ...
                  <input
                    type="password"
                    className={
                      this.state.passCheck2 === null
                        ? "form-control"
                        : this.state.passCheck2 === true
                        ? "form-control border border-success"
                        : this.state.passCheck2 === false
                        ? "form-control border border-danger"
                        : "form-control"
